...
if __name__ == "__main__":
    ...
    parser.add_argument('--class_mapping', nargs='+', type=str, required=False)
    parser.add_argument('--category_names', nargs='+', type=str)
    args = parser.parse_args()

    if args.class_mapping is None:

        c = Converter(os.path.join(args.labeling_folder, 'config.xml'))

        c.convert_to_coco(os.path.join(args.labeling_folder, 'completions'), 
                args.export_folder, 
